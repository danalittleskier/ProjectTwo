...
CREATE TABLE `User_Table` (
  ...
  `user_fullname` VARCHAR(175) NOT NULL,
...
CREATE TABLE `Tool_Table` (
  ...
  `tool_category` ENUM('Power Tools', 'Abrasives', 'Masonry'),
  `tool_price($)` DECIMAL(7,2),
  `tool_owner` INT,

  PRIMARY KEY (`tool_id`),
  FOREIGN KEY (`tool_owner`) REFERENCES `User_Table`(`user_id`)
);
...
CREATE TABLE `Request_Header` (
  ...
  FOREIGN KEY (`requested_by`) REFERENCES `User_Table`(`user_id`),
  FOREIGN KEY (`requested_to`) REFERENCES `User_Table`(`user_id`)
);
...
CREATE TABLE `Request_Line` (
  ...
  FOREIGN KEY (`tool_id`) REFERENCES `Tool_Table`(`tool_id`)
);
...
CREATE TABLE `Tool_Ledger_Entry` (
  ...
  FOREIGN KEY (`tool_owner`) REFERENCES `User_Table`(`user_id`),
  FOREIGN KEY (`tool_borrower`) REFERENCES `User_Table`(`user_id`)
);
